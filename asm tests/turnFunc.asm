...
;* Main code
;***************************************************************
	JUMP TurnMain 	   ; Section for main function for turning
Main:
...
; Function should allow us to turn to face a reflector or object. Assumes sonars are enabled.
TurnMain:

	; Try turning in small segments, wait to reset DTheta until error < 2 Stop anytime if sensor is under threshold
	; Load in Turn Direction (and amount) from TurnDir. -1 = Clockwise, 1 = CCwise
	; Function continually turns in small increments.
TurnStart:
	LOADI Theta
	ADD	  TurnDir
	STORE DTheta
MainTurnLoop:
	IN Theta			; Read current value of Theta from odometry
	SUB DTheta          ; Find odometry - wanted angle
	CALL Abs            ; Find absolute value(current - wanted angle)
	ADDI -2             ; 
	JPOS MainTurnLoop   ; If angle error > 2, keep checking
	
	;JUMP TurnStart		; Uncomment to turn forever
	LOADI TurnDir
	JNEG CwiseCheck
CCWiseCheck
	LOAD DIST0
	SUB  1219 				; Subtract 4 feet or 1219 mm
	JPOS TurnStart 			; If more than 4 feet turn more
	RETURN					; If under 4 feet jump to caller
    ;sonar 0
CWiseCheck
	LOAD DIST5	
	SUB  1219 				; Subtract 4 feet or 1219 mm       
